...
void AudioSource::Play(const bool loop, const float volume) {
  fmodChannel = AudioSystem::PlayFMODSound(fmodSound, loop, volume);
}
...
FMOD::System* AudioSystem::fmodSystem;
std::string AudioSystem::soundFilesRoot;
std::vector<AudioSource*> AudioSystem::audioSources;

void AudioSystem::Init(const std::string&) {
  fmodSystem = nullptr;
  FMOD::System_Create(&fmodSystem);
  fmodSystem->init(512, FMOD_INIT_NORMAL, nullptr);
  const std::string filesRoot =
      R"(Resources\Sound\)";  // TODO: Set this in engine config
  soundFilesRoot = filesRoot;
}
...
AudioSource* AudioSystem::LoadSound(const std::string& soundName) {
