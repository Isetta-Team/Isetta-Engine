...
#define FORWARD CIRCLE + CIRCLE_VERTICIES
    ...
    void DebugDraw::DrawWireSphere(const Math::Matrix4& transformation,
        ...
        OpenGLDraw(transformation * Math::Matrix4::xRot90, color, thickness, depthTest, []() {
            glDrawElements(GL_LINE_LOOP, CIRCLE_INDICIES, GL_UNSIGNED_INT,
                (void*)(sizeof(int) * (PLANE_INDICIES + CUBE_INDICIES +
                    CUBE_WIRE_INDICIES)));
        });
    ...
    void DebugDraw::DrawWireCapsule(const Math::Matrix4& transformation,
        ...
        OpenGLDraw(
            transformation * up * scale * Math::Matrix4::xRot90, color, thickness, depthTest, []() {
            glDrawElements(GL_LINE_LOOP, CIRCLE_INDICIES, GL_UNSIGNED_INT,
                (void*)(sizeof(int) * (PLANE_INDICIES + CUBE_INDICIES +
                    CUBE_WIRE_INDICIES)));
        });
        OpenGLDraw(transformation * down * scale * Math::Matrix4::xRot90, color, thickness,
            depthTest, []() {
            glDrawElements(
